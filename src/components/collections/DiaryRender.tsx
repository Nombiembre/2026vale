...
const monthSlug = (monthIndex: number) => {
  return [
    'enero',
    'febrero',
    'marzo',
...
const DiaryRender: React.FC<DiaryRenderProps> = ({ collection }) => {
  ...
  const filteredYears =
    selectedYear === 'all'
      ? years
      : years.filter((y) => y === parseInt(selectedYear))
        ...
        {filteredYears.map((year) => {
          ...
          const monthIndexes = Array.from(months.keys())
            ...
            .filter((month) => {
              ...
              return monthSlug(month) === selectedMonth
            })
          return (
            <section key={year}>
              <div className="text-center mb-16">
                ...
                <div className="flex justify-center">
                  <div className="w-48 h-px bg-linear-to-r from-transparent via-primary/40 to-transparent" />
                </div>
              </div>
